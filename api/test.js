...
async function addAnalysisToEntries(entries) {
  const analyzedEntries = await entries.map(async (entry) => {
    return await addAnalysisToEntry(entry)
  })
...
async function addAnalysisToEntry(entry) {
  const analysis = await analyzeEntry(entry)
  entry.analysis = analysis
  return entry
}

const analyzedData = await addAnalysisToEntries(sampleData)
